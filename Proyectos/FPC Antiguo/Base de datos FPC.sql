...
ALTER TABLE tipos_identificacion ADD COLUMN activo BOOLEAN DEFAULT TRUE;

ALTER TABLE roles ADD COLUMN descripcion VARCHAR(255);
UPDATE usuarios SET estado = 'activo' WHERE estado = 'ACTIVO';
UPDATE usuarios SET estado = 'inactivo' WHERE estado = 'inactivo';

DESCRIBE fullpaintt.usuarios;
...
SELECT usuario_id, nombre_completo, estado
  FROM usuarios;
...
-- 3. Volver a habilitar safe update
...
DESCRIBE usuarios;
SELECT usuario_id, nombre_completo, estado FROM usuarios;
