 ...
 function nextStep() {
    ...
    // Calcular el total y establecerlo en el campo oculto
    calcularTotal();
}
...
function calcularTotal() {
    ...
    cart.forEach(item => {
        total += parseFloat(item.price.replace(/\./g, '')); // Asegúrate de que el precio esté en el formato correcto
    });

    document.getElementById('total').value = total.toFixed(2); // Establece el total en el campo oculto
}
